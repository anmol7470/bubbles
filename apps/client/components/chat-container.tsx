...
export function ChatContainer({ chatId, user }: { chatId: string; user: User }) {
  ...
  //   noClick: true,
  //   noKeyboard: true,
  //   disabled: selectedImages.length >= 5 || isSendingMessage || isUploadingImages,
  // })
  ...
  return (
    <div
      // {...getRootProps()}
      className={cn(
        'bg-muted dark:bg-background relative flex flex-1 flex-col overflow-hidden'
        ...
        : chat && (
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="bg-background/80 flex h-14 items-center gap-3 border-b border-neutral-300 px-4 dark:border-zinc-800">
                <Link className="flex items-center gap-2" href={`/chats/${chatId}/settings`}>
                  {chat.type === 'groupchat' ? (
              ...
              chatId={chatId}
              participants={chat?.members.map((m) => m.user?.id).filter((id): id is string => !!id) ?? []}
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 px-1">
                  {previewUrls.map((previewUrl, index) => (
                    ...
                    >
                      <Image src={previewUrl} alt={`Selected image ${index + 1}`} fill className="object-cover" />
                      <Button
                        type="button"
                        size="icon"
                        variant="destructive"
                ...
                <div className="flex items-center gap-2">
                  ...
                  /> */}
                    <Textarea
                      ...
                      rows={1}
                      // placeholder={isUploadingImages ? 'Uploading images...' : 'Type a message...'}
